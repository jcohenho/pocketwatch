            ...
            Meteor.http.post(url,data, function( err, res ) {
                if (res.statusCode === 200) {
                    ...
                    for (var i = 0; i < Object.keys(officials_data).length; i++) {
                        var p_index = 'P' + i;
                        Officials.insert({ name: officials_data[p_index].name, address: officials_data[p_index].address, channels: officials_data[p_index].channels, party: officials_data[p_index].party, photo: officials_data[p_index].photoUrl });
                    }
                }
            });
        }
